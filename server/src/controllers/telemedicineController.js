...
exports.getDoctors = async (req, res) => {
  ...
  if (!doctors.length) {
    await Doctor.insertMany([
      { name: 'Ananya Mehra', specialization: 'General Medicine', available: true, rating: 4.7, slots: ['10:00', '11:30', '15:00'] },
      { name: 'Rohit Sinha', specialization: 'Cardiology', available: true, rating: 4.8, slots: ['09:00', '13:30'] },
    ]);
...
exports.bookAppointment = async (req, res) => {
  ...
  const appointment = await Appointment.create({ patient: patient._id, doctor: doctor._id, slot, videoRoomId: crypto.randomUUID() });
